...
struct TCPSocket {
    ...
    TCPSocket(TCPSocket&& other) noexcept : socket_num(other.socket_num), timeout_ms(other.timeout_ms) {
        other.socket_num = INVALID_SOCKET;
    }

    void server_init(int port) {

        // allow immediate rebind after a previous process/socket left TIME_WAIT
        int reuse = 1;
        setsockopt(this->socket_num, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse));
        ...
        while (::bind(this->socket_num, (struct sockaddr *)&server, sizeof(server)) != 0) {
